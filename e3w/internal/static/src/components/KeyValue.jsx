...
const KeyValue = React.createClass({
    ...
    _parseList(list) {
        ...
        list.sort((l1, l2) => {
            return l1.is_dir === l2.is_dir ? l1.key > l2.key : l1.is_dir ? -1 : 1
        })
                    ...
                    <Breadcrumb>
                        {
                            this.state.menus.map(
                                m => (
                                    <Breadcrumb.Item key={m.path} onClick={() => this._redirect(m.path)}><a>{m.name}</a></Breadcrumb.Item>)
                            )
                        }
                    </Breadcrumb>
                </Box>
                <Box>
                    <Box vertical style={{width: 400, paddingRight: 20}}>
                        <Box vertical>
                            {
                                this.state.list.map(
                                    l => (<KeyValueItem key={l.key} enter={this._enter} set={this._set}
                                                        unset={this._unset} info={l}/>)
                                )
                            }
                        </Box>
                    </Box>
                    ...
                    {this.state.setting ?
                        (<CommonPanel hint={currentKey} color="#cce4f6"><KeyValueSetting currentKey={currentKey}
                                                                                         delete={this._delete}/></CommonPanel>) :
                        (<CommonPanel hint="CREATE"><KeyValueCreate update={this._update} back={this._back}
                                                                    dir={this.state.dir}
                                                                    fullKey={this._fullKey}/></CommonPanel>)}
                </Box>
            </Box>
        )
    }
